...
terok sits on top of a stack of sibling packages:

- **terok** — orchestration and instrumentation: project/task lifecycle,
  agent configuration, instructions, CLI and TUI.
...
- **[terok-sandbox](https://github.com/terok-ai/terok-sandbox)** — hardened
  ...
  access, credential vault and SSH key provisioning; live allow/deny
  prompts come from
...
```text
~/.local/share/terok/sandbox-live/tasks/<project>/<task_id>/workspace-dangerous/
```
...
sequenceDiagram
    ...
    Note over U,G: Initial sync (terok project gate-sync)
...
- Container has SSH access via the vault's SSH signer
...
| Option | Effect |
|--------|--------|
| SSH key registered via `ssh-init` | Vault SSH signer available in gatekeeping. Useful for private submodules. **Risk:** if the key has write access to upstream, the agent could bypass the gate. |
...
=== "Gatekeeping"
    ...
    | | `Task —SSH signer→ Upstream` | **Risk:** if key has push access, agent can bypass gate |

---

## SSH Keys and Who Knows What

SSH keys control access to private git repositories. terok mints a
separate key per project (`terok project ssh-init`) into the vault
credential database — there are no on-disk key files.  Keys remain on
the host and are served to containers via the vault's SSH signer.

```mermaid
graph TB
    subgraph HOST ["Host"]
        SSH["Per-project SSH key<br/><i>vault credential DB (host-only)</i>"]
        PROXY["Vault SSH Signer"]
        GATE["Git Gate"]
    end
...
|------|----------------------|--------------------------|
| **Online** | Yes — via vault SSH signer | Yes — push via SSH |
| **Gatekeeping** | No (default) | No — push only to gate via HTTP |
| **Gatekeeping + SSH key** | Yes — via vault SSH signer (opt-in) | Potentially — depends on key permissions |

The gate itself always has access to the SSH key (it needs it to sync with
upstream). The key question is whether the **container** also has SSH signer
access. Private keys never enter the container — the vault signs on behalf
of the container.
...
```text
~/.local/share/terok/sandbox-live/mounts/
...
SSH keys are scoped per project and live in the vault credential
database on the host — no on-disk key files.  Each project's key is
minted by `terok project ssh-init`; private keys never leave the host,
and containers get signatures through the vault SSH signer.
...
```text
~/.local/share/terok/sandbox-live/tasks/<project>/<task_id>/
