...
- When you run a task (CLI or UI), codexctl starts a container and mounts a small set of host directories into it. This enables:
  ...
  - Shared credentials/config for Claude Code under /home/dev/.claude (CLI mode only)
...
- Under this base, three subdirectories may be used:
  ...
  2) _claude-config (required for Claude Code; created automatically if missing)
     - Mounted as: <base_dir>/_claude-config → /home/dev/.claude:Z (read‑write, CLI only)
     - Purpose: Shared Claude Code configuration and auth state. codexctl sets CLAUDE_CONFIG_DIR=/home/dev/.claude for CLI tasks.
...
- /home/dev/.claude       ← <envs_base>/_claude-config:Z (CLI only)
...
- The .claude directory is mounted read‑write for CLI tasks and stores Claude Code config/state (CLAUDE_CONFIG_DIR).
- CLI and UI containers start with the working directory set to /workspace; UI mode does not mount .claude.
