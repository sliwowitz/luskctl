...
Layers
0. L0 — development base (terok-l0:<base-tag>)
...
1. L1 — agent images (terok-l1-cli:<base-tag>, terok-l1-ui:<base-tag>)
   - Built FROM L0.
...
2. L2 — project images (<project>:l2-cli, <project>:l2-ui)
   ...
   - The UI backend is configurable (Codex, Claude, or Mistral). Precedence (highest to lowest):
     1. CLI flag: `terokctl task run-ui --backend <backend>`
     2. Environment variable: `DEFAULT_AGENT` on the host
     3. Per-project config: `default_agent` in project.yml
     4. Global config: `default_agent` in ~/.config/terok/config.yml
     5. Default: codex
     - For Claude, provide TEROK_UI_CLAUDE_API_KEY (or ANTHROPIC_API_KEY / CLAUDE_API_KEY) and optional TEROK_UI_CLAUDE_MODEL.
     - For Mistral, provide TEROK_UI_MISTRAL_API_KEY (or MISTRAL_API_KEY) and optional TEROK_UI_MISTRAL_MODEL.

Build flow
- terokctl generate <project> renders four Dockerfiles (L0/L1/L2) into the per‑project build directory:
  - L0.Dockerfile
  - L1.cli.Dockerfile
  - L1.ui.Dockerfile
  - L2.Dockerfile
- terokctl build <project> builds only the L2 project images (reuses existing L0/L1 images):
  1) <project>:l2-cli FROM terok-l1-cli:<base-tag> (via --build-arg BASE_IMAGE=...)
  2) <project>:l2-ui FROM terok-l1-ui:<base-tag> (via --build-arg BASE_IMAGE=...)
  3) Optional: <project>:l2-dev FROM terok-l0:<base-tag> (when `terokctl build --dev` is used)
- terokctl build --agents <project> rebuilds L0+L1+L2 with fresh agent installs:
  1) terok-l0:<base-tag> FROM docker.base_image (default: Ubuntu 24.04)
  2) terok-l1-cli:<base-tag> FROM terok-l0:<base-tag> — with cache bust to force fresh agent downloads
  3) terok-l1-ui:<base-tag> FROM terok-l0:<base-tag>
  4) <project>:l2-cli FROM terok-l1-cli:<base-tag>
  5) <project>:l2-ui FROM terok-l1-ui:<base-tag>
  6) Optional: <project>:l2-dev FROM terok-l0:<base-tag> (when `--dev` is used)
...
- terokctl build --full-rebuild <project> does a complete rebuild with no cache:
...
- terokctl task run-cli starts <project>:l2-cli; terokctl task run-ui starts <project>:l2-ui.
- Both modes:
...
GPU support
- GPU passthrough is opt‑in per project (run.gpus in project.yml). When enabled, terok adds the necessary Podman flags for NVIDIA GPUs.
