...
def test_list_classifies_runtime_endpoints_separately(
    ...
    """The list separates installable harnesses from runtime endpoints."""
    names = ("opencode", "blablador", "example", "gh")
    ...
    roster.providers = {
        ...
        "example": SimpleNamespace(serves={"openai-chat": "/v1"}),
    }
    ...
    roster.auth_providers.update(
        {
            ...
            "example": SimpleNamespace(label="Example AI"),
        }
    )
    ...
    assert "Select LLM endpoint providers with --provider" in out
    assert "Do not add LLM endpoint providers to image.agents" in out
...
def test_list_falls_back_to_auth_provider_label(capsys: pytest.CaptureFixture[str]) -> None:
    """The list gets the label from ``auth_providers`` when ``agents`` has no entry."""
