...
class TestBundledDefault:
    ...
    def test_bundled_default_contains_key_sections(self) -> None:
        ...
        assert "Classifying internal files by project" in text
...
class TestResolveInstructions:
    ...
    def test_per_provider_dict(self) -> None:
        ...
        assert resolve_instructions(config, "claude") == "Claude instructions"
        assert resolve_instructions(config, "codex") == "Codex instructions"
    ...
    def test_per_provider_dict_fallback_to_default(self) -> None:
        ...
        assert resolve_instructions(config, "vibe") == "Default instructions"
    ...
    def test_per_provider_dict_list_with_inherit(self) -> None:
        ...
        assert resolve_instructions(config, "claude") == f"{default}\n\nTeam policy."
    ...
    def test_per_provider_dict_bare_inherit_returns_default(self) -> None:
        ...
        default = bundled_default_instructions()
        assert resolve_instructions(config, "claude") == default
        assert resolve_instructions(config, "codex") == "Codex custom"
...
class TestFileAppend:
    ...
    def test_absent_yaml_with_file_appends(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert result.endswith("Project notes.")
    ...
    def test_inherit_list_with_file(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            default = bundled_default_instructions()
            assert result.startswith(default)
            assert result.endswith("File content.")
    ...
    def test_inherit_plus_extra_plus_file(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert "Extra YAML." in result
            assert result.endswith("File text.")
...
class TestHasCustomInstructions:
    ...
    def test_false_when_none(self) -> None:
        ...
        assert not has_custom_instructions({"instructions": None})

    def test_true_for_dict_form(self) -> None:
        """Returns True for per-provider dict form."""
        assert has_custom_instructions({"instructions": {"claude": "Custom"}})

    def test_true_for_list_form(self) -> None:
        """Returns True for list form."""
        assert has_custom_instructions({"instructions": ["Part 1", "Part 2"]})
