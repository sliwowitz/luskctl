...
class _TuiLogFormatter:
    ...
    def _format_tool_input(self, tool_input: dict | str) -> list[Text]:
        out: list[Text] = []
...
class LogViewerScreen(screen.Screen[None]):
    ...
    def compose(self) -> ComposeResult:
        mode_label = "FOLLOW" if self.follow else "STATIC"
        yield Static(
            f" Task {self.task_id} ({self.mode}) | {self.container_name} [{mode_label}]",
            id="log-header",
        )
        yield RichLog(auto_scroll=self.follow, id="log-view")
        yield Static(" [Esc/q] Back  [f] Toggle follow  [c] Clear", id="log-footer")
    ...
    def _update_footer_static(self) -> None:
        ...
        def _update() -> None:
            try:
                ...
                footer.update(" [Esc/q] Back  [f] Toggle follow  [c] Clear  [STATIC]")
                log_widget = self.query_one("#log-view", RichLog)
